...
class TaskRepositoryMongoTest(TaskRepositoryTest):
    def setUp(self):
        ...
        self.task_repository = TaskRepositoryMongo(self.mongo_client)


    def test_task_repository_should_assign_unique_id_after_adding_task(self):
        self.mongo_client.submit_task.return_value = [self.a_task]
    ...
    def test_task_repository_should_return_task_list(self):
        self.mongo_client.submit_task.return_value = [self.a_task]
        self.mongo_client.query_multiple.return_value = [self.a_task]
    ...
    def test_task_repository_should_maintain_different_task_lists_for_different_users(self):
        self.mongo_client.submit_task = MagicMock(side_effect=[[self.a_task], [self.other_task]])
        self.mongo_client.query_multiple = MagicMock(side_effect=[[self.a_task], [self.other_task], [self.a_task], [self.other_task]])
